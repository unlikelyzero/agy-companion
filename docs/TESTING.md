...
This plugin was originally built and unit-tested in an environment with no `agy` binary and no Google account available. That gap has been partially closed: a real, authenticated `agy` install (see [`.github/agy-tested-version`](../.github/agy-tested-version) for the exact version last checked), authenticated via the browser/paste-code OAuth flow, ran `node scripts/agy-companion.mjs review --wait --scope working-tree` and `node scripts/agy-companion.mjs adversarial-review --wait --scope working-tree` against a real (small) working-tree diff and produced correctly rendered, schema-conformant results end to end — confirming `--output-format json --json-schema <path>` genuinely works as documented in the README, not just against mocked `spawn` output.

Those runs do not cover everything, though — both were in the foreground, against an account that was already logged in. The rest of this list is still open:

- **The OAuth-prompt detection regex** (`AUTH_REQUIRED_PATTERN` / `URL_PATTERN` in `scripts/lib/agy.mjs`), against a real "Authentication required" prompt. The account used for the live tests above was already authenticated, so this path still hasn't fired against real agy output — only against fixtures in the unit tests.
...
- **Background job execution** (`--background` on `review`, `adversarial-review`, and `task`): the detached-worker spawn/poll/tail-file mechanics are exercised by the unit tests for their pieces, and the live tests above ran in the foreground, so the background path hasn't been proven against a real `agy` process yet.

If you're picking this up with a working `agy` install: run through `/agy:setup` and `/agy:rescue --wait "say hi"` next (both review foreground paths are now confirmed), then try the background variants and `--resume`/`--conversation`.
