...
- **What's been live-tested vs. what's still mock-only.** Confirmed against a real, authenticated `agy` install: binary/version detection, and full `/agy:review --scope working-tree` and `/agy:adversarial-review --scope working-tree` runs producing valid structured output end to end. Still only unit-tested against a mocked `child_process.spawn`, not a live process: the OAuth-required detection path (the test account was already logged in), background job execution and `/agy:status` polling on a real running job, `--write` task execution and touched-files detection, and `--resume`/`--continue`/`--conversation` behavior. See [docs/TESTING.md](docs/TESTING.md) for the current split and what to smoke-test next.
