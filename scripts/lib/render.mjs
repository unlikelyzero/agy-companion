...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.conversationResumable) {
    lines.push("  Resumable: yes, via `/agy:rescue --resume` (continues agy's most-recent conversation, not a targeted per-job thread)");
  }
...
function renderReviewResultBody(parsedResult, meta) {
  if (!parsedResult.parsed) {
    ...
    if (parsedResult.rawOutput) {
      lines.push("", "Raw final output:", "", "```text", parsedResult.rawOutput, "```");
    }
...
export function renderStatusReport(report) {
  const lines = ["# agy Status", "", `Session runtime: ${report.sessionRuntime.label}`, `Review gate: ${report.config.stopReviewGate ? "enabled" : "disabled"}`, ""];
...
export function renderStoredJobResult(job, storedJob) {
  ...
  const lines = [`# ${job.title ?? "agy Result"}`, "", `Job: ${job.id}`, `Status: ${job.status}`];
